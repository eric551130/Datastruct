...
int visit(long int maze[][size],int i, int j)
{
	...
	if(success != 1 && maze[i][j+1] == 0) visit(maze,i+1, j+1); //右下  
    if(success != 1 && maze[i+1][j] == 0) visit(maze,i+1, j); //下 
    if(success != 1 && maze[i+1][j] == 0) visit(maze,i+1, j-1); //左下 
    if(success != 1 && maze[i][j-1] == 0) visit(maze,i, j-1); //左
	if(success != 1 && maze[i][j-1] == 0) visit(maze,i-1, j-1); //左上 
    if(success != 1 && maze[i-1][j] == 0) visit(maze,i-1, j); //上 
 	if(success != 1 && maze[i-1][j] == 0) visit(maze,i-1, j+1); //右上
